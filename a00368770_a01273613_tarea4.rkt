...
;4
;función auxiliar que ayuda a desplegar y concatenar el nivel actual del
;nodo del arbol
(define nivel-aux
  (lambda (arbol cont)
    (if (null? arbol) '()
...
;5
;función principal que busca primero las hojas y después regresa su valor
;para que se sume con su nodo padre
(define (acumulado arbol)
  (cond ((null? arbol) '(0));fix para poder desplegar de la forma correcta los resutlados
        ((equal? (cdr arbol) '(() ())) arbol)
        (else (list(+ (car arbol) (car(acumulado (cadr arbol))) (car(acumulado (caddr arbol)))) (acumulado (cadr arbol)) (acumulado (caddr arbol))))))
...
;7
(define prueba '((X 0 5) (Y 0 2)))
(define (suma-col grafo)
  (apply map + (map cdr grafo)))
(define (tour-aux grafo)
  (cond ((null? grafo) #t)
        ((zero? (car grafo)) #f)
        (else (tour-aux (cdr grafo)))))
(define tour?
  (lambda (grafo)
        (tour-aux (suma-col grafo))))
;8
(define graf
'((A (B 2) (D 10))
(B (C 9) (E 5))
...
(define (member? item seq)
  (sequence-ormap (lambda (x)
                    (equal? item x))
                  seq))
(define (cuenta-grafo-nodos grafo)
  (cond ((null? grafo) 0)
        (else (+ 1 (cuenta-grafo-nodos (cdr grafo))))))
(define (adyacentes grafo e)
  (cond ((null? grafo) 0)
        ((equal? e (caar grafo)) (cuenta-grafo-nodos (apply append (map cdr (cdar graf)))))
        (else (adyacentes (cdr grafo) e))))
(define (cantidad-adyacentes grafo e)
  (cond ((null? grafo) '())
        ((member? e (grafo)) (adyacentes grafo e))
        (else '=>error)))
